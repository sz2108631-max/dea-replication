...
供应链韧性采用**深度自编码器 + Sigma 地标 + TOPSIS**三步法从企业财务与专利数据中无监督构建：

```
11个三级指标 → 自编码器降维(11→8→6→4) → Sigma地标选择(γ=ρ×δ≥μ+2σ) → 异常值剔除 → TOPSIS评分 → res_v2
```
...
- **Sigma 地标**：γ = ρ × δ ≥ μ + 2σ，同时考虑密度和分离度，去冗余后剔除异常值（> μ + 2.5σ）
