...
# shandian 已通过 02_prepare_panel.py 合并入面板
df["ln_lightning"] = np.log(df["shandian"].fillna(0) + 1)
n_merged = df["shandian"].notna().sum()
print(f"雷电数据(shandian): {n_merged:,} / {len(df):,} obs ({100*n_merged/len(df):.1f}%)")
...
def make_sample(data, extra_drop=None):
    cols = (["Dea", "Breadth", "Depth"] + CONTROLS + [OUTCOME]
            + ["stkcode", "year", "city", "shandian", "ln_lightning"]
            + (extra_drop or []))
    available = [c for c in cols if c in data.columns]
    d = data[available].copy()
    d = d.dropna(subset=["Dea"] + CONTROLS + [OUTCOME, "shandian"])
