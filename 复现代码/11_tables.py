...
ALL_CONTROLS = ["lnage", "klr", "lnsize", "bsize", "dual", "lnrd", "indrate", "own", "lev"]
CONTROL_LABELS = {
...
def make_sample(data, extra_vars=None, controls=None):
    """构建样本。controls=None时用CONTROLS(3个); 可传入ALL_CONTROLS用于逐步回归"""
    ctl = controls if controls is not None else CONTROLS
    cols = (["Dea", "Breadth", "Depth"] + ctl + [OUTCOME, "stkcode", "year", "city"]
            + (extra_vars or []))
    available = [c for c in cols if c in data.columns]
    d = data[available].copy()
    # 用实际的CONTROLS(3个)做dropna，但保留额外控制变量的列
    core_cols = [c for c in ["Dea"] + CONTROLS + [OUTCOME, "city"] if c in d.columns]
    return d.dropna(subset=core_cols)
...
s_stepwise = make_sample(df, controls=ALL_CONTROLS)  # 逐步回归需包含全部控制变量
run_and_print_table("表1 Panel A: Dea 逐步加入控制变量",
    [
        ("(1) 无控制",     ["Dea"], lambda s: s),
        ("(2) +公司特征",  ["Dea", "lnage", "lnsize", "lev"], lambda s: s),
        ("(3) +治理结构",  ["Dea", "lnage", "lnsize", "lev", "bsize", "dual", "indrate", "own"], lambda s: s),
        ("(4) +研发投入",  ["Dea", "lnage", "lnsize", "lev", "bsize", "dual", "indrate", "own", "lnrd"], lambda s: s),
        ("(5) 全部控制",   ["Dea"] + CONTROLS, lambda s: s),
    ],
...
# Merge IC and Cost data (skip if already in panel)
df_mech = df.copy()

if "lnIC" not in df_mech.columns:
    ...
    df_mech = df_mech.merge(ic_raw[["stkcode", "year", "lnIC"]], on=["stkcode", "year"], how="left")
    print(f"  [机制数据] lnIC merge (from raw): {df_mech['lnIC'].notna().sum():,} / {len(df_mech):,}")
else:
    print(f"  [机制数据] lnIC already in panel: {df_mech['lnIC'].notna().sum():,} / {len(df_mech):,}")

if "cost" not in df_mech.columns:
    ...
    print(f"  [机制数据] cost merge (from raw): {df_mech['cost'].notna().sum():,} / {len(df_mech):,}")
else:
    print(f"  [机制数据] cost already in panel: {df_mech['cost'].notna().sum():,} / {len(df_mech):,}")
...
def make_sample_mech(data, extra_vars=None):
    cols = (["Dea", "Breadth", "Depth", "lnIC", "cost"]
            + CONTROLS + [OUTCOME, "stkcode", "year", "city"]
            + (extra_vars or []))
    available = [c for c in cols if c in data.columns]
    d = data[available].copy()
...
# Build heterogeneity dimensions from available columns
hetero_dims = {}

# H1: 区域 (use area_1: 1=东部, 2=中部, 3=西部)
if "area_1" in df.columns:
    area_map = {1: "东部", 2: "中部", 3: "西部"}
    for val, name in area_map.items():
        hetero_dims.setdefault("H1_区域", {})[name] = lambda s, v=val: s[s["area_1"] == v]

# H2: 产权 (use soe column if available)
if "soe" in df.columns:
    soe_map = {0: "民营", 1: "国有"}
    for val, name in soe_map.items():
        hetero_dims.setdefault("H2_产权", {})[name] = lambda s, v=val: s[s["soe"] == v]

# H3: 市场竞争 (use hhi_d)
if "hhi_d" in df.columns:
    hetero_dims["H3_市场竞争"] = {
        "高竞争(低HHI)": lambda s: s[s["hhi_d"] <= s["hhi_d"].median()],
        "低竞争(高HHI)": lambda s: s[s["hhi_d"] > s["hhi_d"].median()],
    }

# H4: 高科技 (use hightech column)
if "hightech" in df.columns:
    tech_map = {0: "非高科技", 1: "高科技"}
    for val, name in tech_map.items():
        hetero_dims.setdefault("H4_高科技", {})[name] = lambda s, v=val: s[s["hightech"] == v]

# H6: 环境不确定性
if "env_unc" in df.columns and df["env_unc"].notna().sum() > 500:
    s_env = df.dropna(subset=["env_unc"])
    med_env = s_env["env_unc"].median()
    hetero_dims["H6_环境不确定性"] = {
        "低不确定性": lambda s: s[s["env_unc"].dropna() <= s["env_unc"].median()] if s["env_unc"].notna().any() else s,
        "高不确定性": lambda s: s[s["env_unc"].dropna() > s["env_unc"].median()] if s["env_unc"].notna().any() else s,
    }
