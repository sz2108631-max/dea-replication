...
# 同时保存 11 个三级指标（供二级指标回归使用）
# sub 包含原始缩尾正向化前指标，保存标准化后的指标
ind_df = sub[['stkcode','year']].copy()
for j, col in enumerate(ind_cols):
    ind_df[col] = X_raw[:, j]  # 缩尾+正向化后、标准化前
ind_df.to_csv(intermediate / "indicators_11.csv", index=False)
pyreadstat.write_dta(ind_df, intermediate / "indicators_11.dta")
print(f"  已保存: {len(ind_df):,} obs to indicators_11.dta (11指标)")
