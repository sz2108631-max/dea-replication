...
DATA_AI = Path("/Users/weixuan/Desktop/论文/数据���产与生成式AI创新")
OUT = ROOT / "output"
...
df_base = df[["res", "Dea", "id_stock", "year"] + use_ctrl].dropna().copy()
print(f"基准回归样本: {len(df_base):,} obs, {df_base['id_stock'].nunique():,} firms\n")
...
d1 = df_base.merge(fin[["id_stock","year","债券融资规模","股权融资规模","商业信用融资规模"]],
...
d1b = df_base.merge(cred[["id_stock","year","信贷可得性1","信贷可得性2"]],
               on=["id_stock","year"], how="left")
...
d2 = df_base.merge(cfvol[["id_stock","year","现金流波动"]], on=["id_stock","year"], how="left")
...
# Linear + Quadratic (same sample as benchmark)
