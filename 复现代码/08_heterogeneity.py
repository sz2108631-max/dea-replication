...
DATA = ROOT / "data"
ENV_FILE = DATA / "环境不确定性1999-2024.dta"

# ── 读取数据（area_1, soe, hhi_d, hightech 已在 DTA 中）───────────────
...
split_results = []
split_vars = [("hhi_d", "HHI市场竞争")]
if "env_unc" in sample.columns and sample["env_unc"].notna().sum() > 500:
    split_vars.append(("env_unc", "环境不确定性"))
for var, label in split_vars:
